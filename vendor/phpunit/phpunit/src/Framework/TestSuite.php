...
class TestSuite implements IteratorAggregate, Reorderable, SelfDescribing, Test
{
    ...
    public function addTestFile(string $filename): void
    {
        ...
        // The given file may contain further stub implementations in addition to the
        ...
        // accumulate all discovered implementations, so the parent class may be found in
        // a later invocation.
        if (!empty($newClasses)) {
            // On the assumption that test implementations are defined first in files,
            // process discovered implementations in approximate LIFO order, so as to
        ...
        if (count($this->tests) > ++$numTests) {
            $this->addWarning(
                sprintf(
                    "Multiple test case implementations per file is deprecated\n               in %s",
                    $filename
                )
            );
        }
